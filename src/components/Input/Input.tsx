...
function InputRoot({ children, className, textArea }: TextInputRootProps) {
    return (
        <div className={clsx('flex items-center Input-Gradient relative w-full z-[1] after:rounded-lg before:rounded-lg', className)}>
            <div className={clsx('flex bg-gray-300 items-center gap-4 w-full ml-1 rounded-md dark:bg-gray-800', textArea ? 'h-full' : 'h-12')}>
                {children}
            </div>
        </div>
    );
...
function InputInput({ PlaceHolderComponent, ...rest }: TextInputInputProps) {
    ...
    return (
        <input
            className='bg-transparent flex-1 caret-fuchsia-500 text-stone-900 2xl:text-md outline-none placeholder:text-gray-500 dark:text-white dark:placeholder:text-gray-400'
            placeholder={
                language === "English"
                    ?
...
function InputArea({ PlaceHolderComponent, ...rest }: InputAreaProps) {
    ...
    return (
        <textarea
            className="resize-y max-h-[300px] min-h-[120px] 2xl:text-md caret-fuchsia-500 bg-transparent w-full py-[6px] placeholder:text-gray-500 text-stone-900 h-[120px] px-2 outline-0 dark:placeholder:text-gray-400 dark:text-white xs:resize-none"
            placeholder={
                language === "English"
                    ?
                    PlaceHolderComponent?.English
